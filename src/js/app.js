/**
 * Este script contém todos os listeners, botões e callbacks.
 */
var game = new Game();
...
var searchTypeSelectbox = document.getElementById("searchType"); //MUDAR ISSO
...
searchButton.addEventListener(
  ...
  function () {
    ...
    if (isNaN(iterationLimit)) return alert("Invalid iteration limit");

    if (isNaN(depthLimit)) return alert("Invalid depth limit");
...
searchStepButton.addEventListener(
  "click",
  function () {
    Board.clearReplay();

    if (searchStepOptions) return search(searchStepOptions);
...
searchStopButton.addEventListener(
  "click",
  function () {
    Board.clearReplay();
    searchResultDiv.innerHTML = "";
...
function searchCallback(err, options) {
  ...
  searchResultDiv.innerHTML =
    (err ? err : "Solved! Depth: " + options.node.depth) +
    " <br/>" +
    ...
    (err
      ? ""
      : '<br/><br/><button id="replayButton" onclick="replayWinnerNode()">Replay solution</button>');
...
function replayWinnerNode() {
  if (!window.winnerNode) return alert("Winner node could not found");

  if (window.isReplaying) return Board.clearReplay();
